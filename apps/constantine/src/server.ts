...
// davranışıyla anında ölür. Ölçüldü (spare port 4099): SIGINT'te süreç öldü,
// tek satır kapanış logu basılmadı. Bu yüzden İKİ sinyal de karşılanıyor.
...
async function shutdown(signal: string): Promise<void> {
  ...
  try {
    // Zamanlayıcıları önce durdur: kapanış sırasında yeni bir kampanya
    // gönderimi ya da IMAP çekimi başlamasın.
    stopCronScheduler();
    stopCampaignWorker();
    // Handler bunu çağırmıyordu — IMAP bağlantıları açık kalıyordu.
    stopMailcowReplyPoller();

    await new Promise<void>((resolve) => {
      server.close(() => resolve());
      // DİKKAT: server.close() TEK BAŞINA YETMEZ. CRM frontend'i keep-alive
      // soket tutuyor (heartbeat RPC'si var); close() açık soket kaldığı sürece
      // callback'i çağırmaz ve her kapanış yukarıdaki 10sn guard'a düşüp
      // exit(1) ile biterdi — yani "düzgün kapanış" adı altında her seferinde
      // 10sn'lik kirli çıkış. Boştaki keep-alive soketlerini hemen kapat,
      // 3sn sonra hâlâ uçuşta olan istek varsa onu da kes.
