#!/usr/bin/env node
/**
 * RENDER UYGULAMA — karar defterindeki kararlari CRM'e yazar.
 *   node render-uygula.mjs --karar <csv> --kaynak <etiket-metni> [--geri-al]
...
const bayrak = (a) => { const i = process.argv.indexOf(a); return i > 0 ? process.argv[i + 1] : null; };
const KARAR_CSV = bayrak('--karar') || `${DIR}/GOREV2B-KARAR.csv`;
const KAYNAK = bayrak('--kaynak') || 'gorev2b-render-2026-08-26';
const IZ_ETIKET = KAYNAK.toUpperCase();
...
const kararlar = csvOku(KARAR_CSV);
...
for (const r of kararlar) {
  if (r.karar === 'KORU-ETIKETLE') {
    ...
    fs.appendFileSync(`${DIR}/MAIL-DUZELTME-IZI.csv`,
      `\n${r.lead_id},${r.eski_mail},${r.yeni_mail},"${r.firma.replace(/"/g,'""')}",${r.site},${IZ_ETIKET}`);
  }
}
